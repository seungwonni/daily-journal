 ...
 * dir *.exe라고 치면 확장자가 exe인 파일이 다 나온다. "dir 패턴"과 같이 치면 그 패턴에 맞는 파일만 검색 결과로 나온다. 예를 들어,
 * dir a?b.exe라고 검색하면 파일명의 첫 번째 글자가 a이고, 세 번째 글자가 b이고, 확장자가 exe인 것이 모두 나온다.
...
public class Terminal {
    ...
    private static final Integer inputNumber = 2;
    ...
    private static String input() {
        ...
        for (int i = 0; i<inputNumber; i++) {
            System.out.print(i +"번째 문자를 입력하세요." );
            input = scanner.next();
            if (!validate(input)) {
                System.out.println("형식의 맞지 않습니다.(소문자와 '.' 문자만 허용)");
                i--;
                continue;
            }
            inputList.add(input);
